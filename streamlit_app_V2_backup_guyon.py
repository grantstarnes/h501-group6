...
def display_top_genres_by_time_to_beat(game: dict):
    """
    Display a scatter plot showing top 10 genres based on time to beat metrics.
    Shows average hours to complete for each genre across all completion modes.
    ...
    # Get games with time to beat data
    df = games_df[
        (games_df["ttb_hastily"].notna()) | 
        (games_df["ttb_normally"].notna()) | 
        (games_df["ttb_completely"].notna())
    ].copy()
    
    if df.empty:
        st.caption("Not enough time to beat data available.")
        return
    
    # Explode genres (games can have multiple genres)
    ...
    # Calculate average time to beat for each genre and mode
    genre_stats = []
    
    for genre in df_exploded["genre_names"].unique():
        genre_games = df_exploded[df_exploded["genre_names"] == genre]
        
        hastily = genre_games["ttb_hastily"].dropna()
        normally = genre_games["ttb_normally"].dropna()
        completely = genre_games["ttb_completely"].dropna()
        ...
        if len(completely) > 0:
            genre_stats.append({
                "Genre": genre,
                "Mode": "Completely",
                "Hours": completely.mean(),
                "Count": len(completely)
            })
    
    if not genre_stats:
        st.caption("Not enough data to generate genre time to beat chart.")
        return
    
    genre_df = pd.DataFrame(genre_stats)
    
    # Get top 10 genres by total games with TTB data
    top_genres = genre_df.groupby("Genre")["Count"].sum().nlargest(10).index.tolist()
    genre_df = genre_df[genre_df["Genre"].isin(top_genres)]
    
    # Calculate average rating for color mapping
    genre_ratings = []
    for genre in top_genres:
        genre_games = df_exploded[df_exploded["genre_names"] == genre]
        avg_rating = genre_games["aggregated_rating"].mean()
        genre_ratings.append({"Genre": genre, "Average_Rating": avg_rating})
    
    rating_df = pd.DataFrame(genre_ratings)
    genre_df = genre_df.merge(rating_df, on="Genre", how="left")
    ...
    fig = px.scatter(
        ...
        facet_col="Mode",
        labels={
            "Hours": "Average Hours to Complete",
            "Genre": "Genre",
            "Average_Rating": "Average Rating"
        },
        color_continuous_scale="Viridis",
        hover_data={"Count": True, "Hours": ":.1f", "Average_Rating": ":.1f"}
    )
    
    fig.update_layout(
        title="Ratings of the Top 10 Genres based on Time to Beat Mode",
        showlegend=True,
        height=500,
    )
    
    # Update facet titles to remove "Mode ="
...
def main():
    ...
    if st.session_state.phase == "details" and st.session_state.confirmed_game_id is not None:
        ...
        if game_info:
            ...
            # Top Genres by Time to Beat - OPT-IN to avoid memory spike
            ...
            if not st.session_state.show_genre_chart:
                ...
                if st.button("📊 Show Top Genres by Time to Beat", use_container_width=True):
                ...
                st.markdown("### Top Genres by Time to Beat")
                display_top_genres_by_time_to_beat(game_info)
        else:
